...
void Player::Draw(const Camera& camera)
{
	if (isAlive_) {
		if (behavior_ == Behavior::kAttack) {
			models_[4]->Draw(worldTransform_Weapon_, camera);
		}
		models_[0]->Draw(worldTransform_Body_, camera);
		models_[1]->Draw(worldTransform_Head_, camera);
		models_[2]->Draw(worldTransform_Larm_, camera);
	...
	if (Input::GetInstance()->IsControllerConnected()) {
		if (Input::GetInstance()->GetJoystickState(0, joyState)) {
			...
			if (isMoving) {
				...
				move = TransformVector3(move, rotate);
