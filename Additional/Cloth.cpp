...
void Cloth::Initialize(yunity::World* world)
{
	...
	for (uint32_t index = 0; index < 24; index++) {
		lines_[index] = std::make_unique<yunity::PrimitiveDrawer>();
		lines_[index].reset(yunity::PrimitiveDrawer::Create());
		lines_[index]->SetCamera(CameraManager::GetInstance()->GetCamera());
	}

	for (uint32_t x = 0; x < 4; x++) {
		for (uint32_t y = 0; y < 3; y++) {
			joints_[y + x * 3] = std::make_unique<yunity::SpringJoint>();
			...
			joints_[y + 12 + x * 3] = std::make_unique<yunity::SpringJoint>();
...
{
	float force = 10.0f;
	if (yunity::Input::GetInstance()->TriggerKey(DIK_3)) {
		for (uint32_t index = 0; index < 4; index++) {
			points_[index * 4]->AddForce({ force, 0.0f, 0.0f }, yunity::Body::ForceMode::kImpulse);
			points_[index * 4 + 1]->AddForce({ force * 0.8f, 0.0f, 0.0f }, yunity::Body::ForceMode::kImpulse);
			points_[index * 4 + 2]->AddForce({ force * 0.5f, 0.0f, 0.0f }, yunity::Body::ForceMode::kImpulse);
		}
	}
	if (yunity::Input::GetInstance()->TriggerKey(DIK_4)) {
		for (uint32_t index = 0; index < 4; index++) {
			points_[index * 4]->AddForce({ -force, 0.0f, 0.0f }, yunity::Body::ForceMode::kImpulse);
			points_[index * 4 + 1]->AddForce({ -force * 0.8f, 0.0f, 0.0f }, yunity::Body::ForceMode::kImpulse);
			points_[index * 4 + 2]->AddForce({ -force * 0.5f, 0.0f, 0.0f }, yunity::Body::ForceMode::kImpulse);
	...
	if (yunity::Input::GetInstance()->PushKey(DIK_5)) {
		for (uint32_t index = 0; index < 4; index++) {
			points_[index * 4]->AddForce({ 5.0f, 0.0f, 0.0f }, yunity::Body::ForceMode::kForce);
		}
	}
	if (yunity::Input::GetInstance()->PushKey(DIK_6)) {
		for (uint32_t index = 0; index < 4; index++) {
			points_[index * 4]->AddForce({ -5.0f, 0.0f, 0.0f }, yunity::Body::ForceMode::kForce);
		}
	}
