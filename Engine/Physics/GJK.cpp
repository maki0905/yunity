...
void yunity::EPA(Point& a, Point& b, Point& c, Point& d, Object3D* colliderA, Object3D* colliderB, World* world)
{
    ...
    for (int index = 0; index < 64; index++) {
		...
		for (int i = 0; i < numFaces; i++)
		{
			if (Dot(faces[i][3].point, Subtract(p.point, faces[i][0].point)) > 0)
			{
				for (int j = 0; j < 3; j++) 
				{
					...
					{
						if (loose_edges[k][1].point.x == current_edge[0].point.x && loose_edges[k][1].point.y == current_edge[0].point.y && loose_edges[k][1].point.z == current_edge[0].point.z) {
							if (loose_edges[k][0].point.x == current_edge[1].point.x && loose_edges[k][0].point.y == current_edge[1].point.y && loose_edges[k][0].point.z == current_edge[1].point.z) {
		...
		for (int i = 0; i < num_loose_edges; i++)
		{
			...
			if (Dot(faces[numFaces][0].point, faces[numFaces][3].point) + bias < 0) {
