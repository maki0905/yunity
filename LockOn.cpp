...
void LockOn::Update(const std::list<std::unique_ptr<Enemy>>& enemies, const ViewProjection& viewProjection)
{
		...
		if (mode_ == LockOnMode::kManual) {
			if (joyState_.Gamepad.wButtons & XINPUT_GAMEPAD_X) {
				if (!(joyStatePre_.Gamepad.wButtons & XINPUT_GAMEPAD_X)) {
					ReSearch(enemies, viewProjection);
				}
			}
		}
	...
	if (target_) {
		...
		// ワールド座標からスクリーン座標に変換
		Vector3 positionScreen = MapWorldToScreen(positionWorld, viewProjection.matView, viewProjection.matProjection, WinApp::kWindowWidth, WinApp::kWindowHeight);
		// Vector2に格納
...
void LockOn::ReSearch(const std::list<std::unique_ptr<Enemy>>& enemies, const ViewProjection& viewProjection)
{
	// 目標
	std::list<std::pair<float, const Enemy*>> targets;
